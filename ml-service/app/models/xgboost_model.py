...
Trains quantile regression models on close price for 95% confidence intervals:
- median model  (α = 0.5):   point estimate
- lower model   (α = 0.025): 2.5th percentile
- upper model   (α = 0.975): 97.5th percentile

Uses time-based train/test split (no shuffle) to prevent data leakage.
"""
...
class XGBoostPredictor:
    ...
    def train(
        ...
        # Time-based split — critical for time series, never shuffle!
        split_idx = int(len(X) * (1 - test_size))
        X_train, X_test = X[:split_idx], X[split_idx:]
        y_train, y_test = y[:split_idx], y[split_idx:]

        logger.info(
            "Training XGBoost: %d train, %d test samples", len(X_train), len(X_test)
        )
        ...
        self.model_median = XGBRegressor(
            objective="reg:quantileerror", quantile_alpha=0.5, **common_params
        )
        ...
        self.model_lower = XGBRegressor(
            objective="reg:quantileerror", quantile_alpha=0.025, **common_params
        )
        ...
        self.model_upper = XGBRegressor(
            objective="reg:quantileerror", quantile_alpha=0.975, **common_params
        )
        ...
        self.metrics = ModelMetrics(
            ...
            architecture_details={
                ...
                "quantile_heads": [0.025, 0.5, 0.975],
            },
        )
    ...
    def _update_features_for_next_step(
        ...
        Args:
            ...
            new_price: Predicted price for the previous step.
    ...
    def _save(self):
        """Save all three models and metadata to disk."""
        os.makedirs(MODEL_DIR, exist_ok=True)
