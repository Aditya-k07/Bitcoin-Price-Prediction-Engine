...
func (h *Handler) ExportPredictions(c *gin.Context) {
	...
	if err != nil {
		...
		log.Printf("[Handler] ML warmup prediction skipped (export): %v", err)
	}

	content, disposition, err := h.MLClient.DownloadPredictionsExcel(model, days)
	if err != nil {
		log.Printf("[Handler] ML export error: %v", err)
	...
	if disposition == "" {
		disposition = "attachment; filename=predictions.xlsx"
	}
