...
def _try_load_models():
    ...
    prophet_loaded = prophet_predictor.load()

    if xgb_loaded:
        logger.info("XGBoost model loaded from saved state.")
    if prophet_loaded:
        logger.info("Prophet model loaded from saved state.")

    return xgb_loaded, prophet_loaded
...
async def health_check():
    ...
    if prophet_predictor.is_trained:
...
async def predict(
    model: str = Query("xgboost", regex="^(xgboost|prophet)$",
    ...
    - **model**: 'xgboost' (Aggressive) or 'prophet' (Conservative)
    ...
    if model == "xgboost":
        ...
        if not predictor.is_trained:
            logger.info("Prophet not trained, training now...")
            predictor.train(daily_data)

        predictions = predictor.predict(daily_data, days=days)
        metrics = predictor.metrics
...
async def retrain(
    model: str = Query("xgboost", regex="^(xgboost|prophet)$",
                       description="Model to retrain: 'xgboost' or 'prophet'")
):
    """
    ...
    if model == "xgboost":
        ...
        metrics = prophet_predictor.train(_daily_data)
    else:
        raise HTTPException(status_code=400, detail=f"Unknown model: {model}")
